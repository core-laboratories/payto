<script lang="ts">
	...
	function handleCardNumberInput(event: Event) {
		...
		if (luhnValid) {
			...
			const publicLength = Math.min(brandMatch.publicDigits, Math.max(totalLength - lastFourLength, 0));
	...
	async function handleProceed() {
		...
		if (bin6.length < 6 || last4.length < 4) {
			return;
		}
						...
						<label class="flex flex-col gap-1 w-3/4">
							...
							<input
								...
								oninput={(event) => {
									...
									const lettersOnlyLength = sanitized.replace(/ /g, '').length;
									if (!sanitized.length) {
										cardholderValidationState = 'empty';
									} else if (lettersOnlyLength >= 2) {
